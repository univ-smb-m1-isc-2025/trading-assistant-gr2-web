...
import './Register.css'; // Optional: for basic styling
...
const Register: React.FC = () => { // Use React.FC for functional components
    ...
    const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await axios.post<RegistrationSuccessResponse>(API_URL, formData, {
                headers: {
            ...
            setMessage(response.data.message || `Registration successful for ${response.data.username}!`);
            setFormData({ username: '', email: '', password: '' }); // Clear form on success

        } catch (err) {
            console.error('Registration error:', err);
            setError('Registration failed. Please check the details below or try again.'); // Default error
                ...
                if (axiosError.response) {
                    ...
                    if (status === 400 && typeof data === 'object' && data !== null) {
                        ...
                        if (typeof data === 'string') {
                            setError(data);
                        } else if (typeof data === 'object' && data !== null && (data as ErrorResponse).error) {
                    ...
                    } else if (typeof data === 'object' && data !== null && (data as ErrorResponse).message) {
    ...
    return (
        <div className="register-container">
            ...
            {error && Object.keys(fieldErrors).length === 0 && <p className="error-message">{error}</p>}
